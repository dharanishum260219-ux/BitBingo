...
function SessionHistoryPanel() {
  const { sessions, stopSession, deleteSession } = useArena()

  const handleDeleteSession = async (sessionId: string, sessionName: string) => {
    const confirmed = window.confirm(
      `Delete old session \"${sessionName}\"? This removes its teams, completions, and board mapping.`,
    )
          ...
          {sessions.map((session) => (
            <div key={session.id} className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between py-3 px-2">
              ...
              <div className="flex items-center gap-4 sm:ml-auto">
                ...
                {session.status !== "Active" && (
                  <Btn variant="danger" onClick={() => void handleDeleteSession(session.id, session.name)}>
                    <Trash2 className="w-3 h-3" />
                    DELETE OLD SESSION
                  </Btn>
                )}
